...
def list_plans(
    ...
    *,
    ...
    rows = conn.execute(
        f"""
        select ap.plan_id, ap.rep_id, ap.plan_date,
               ap.start_time, ap.end_time, ap.category, ap.title,
               ap.customer_id, ap.deal_id,
               ap.activity_type, ap.priority, ap.expected_amount, ap.expected_probability,
               ap.plan_status, ap.is_ai_generated, ap.rationale,
               p.product_id, coalesce(ap.product_name_override, p.product_name) as product_name
