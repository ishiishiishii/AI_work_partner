...
```
   ...
   トークンなしでも都営地下鉄・都営バスを利用できます。東京メトロ・東京臨海高速鉄道
   （りんかい線）・首都圏新都市鉄道（つくばエクスプレス）・多摩都市モノレール・
   横浜市営地下鉄も含める場合は、公共交通オープンデータセンターの無料開発者登録後、
   `.env` の`ODPT_ACCESS_TOKEN`へトークンを設定して再実行します。JR東日本・相模鉄道・
   京王電鉄・東武鉄道を含める場合は公共交通オープンデータチャレンジ2026のトークンを
   `ODPT_CHALLENGE_ACCESS_TOKEN`へ設定します。東急・小田急・京急・西武・京成などは
   ODPTがGTFSパッケージを配布していない（駅・経路・ダイヤ等のLinked Dataのみ）ため、
   この方法では取得できません。
